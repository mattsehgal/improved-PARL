...
def user_embeds_model(word_latent_factor, latent_dim, num_filters, windows_size, users_inputs, users_masks_inputs, word_embeddings):
    ...
    #CNN layers
    W = tf.Variable(tf.random.truncated_normal([windows_size, word_latent_factor, 1, num_filters],stddev=0.3), name="user_W")
    ...
    W1 = tf.Variable(tf.random.truncated_normal([num_filters, latent_dim],stddev=0.3), name="user_W1")


    b1 = tf.Variable(tf.constant(0.1, shape=[latent_dim]), name="user_b")

    user_vector = tf.compat.v1.nn.relu_layer(o, W1, b1, name="user_layer1")
...
def item_embeds_model(word_latent_factor, latent_dim, num_filters, windows_size, items_inputs, items_masks_inputs, word_embeddings):
    ...
    #CNN layers
    W = tf.Variable(tf.random.truncated_normal([windows_size, word_latent_factor, 1, num_filters],stddev=0.3), name="item_W")
    b = tf.Variable(tf.constant(0.1, shape=[num_filters]), name="user_b")
    ...
    W1 = tf.Variable(tf.random.truncated_normal([num_filters, latent_dim],stddev=0.3), name="item_W1")


    b1 = tf.Variable(tf.constant(0.1, shape=[latent_dim]), name="item_b")

    item_vector = tf.compat.v1.nn.relu_layer(o, W1, b1, name="item_layer1")
...
def user_aux_embeds_model(word_latent_factor, latent_dim, num_filters, windows_size, users_aux_inputs, users_aux_mask_inputs, word_embeddings):
    ...
    # CNN layers
    W = tf.Variable(tf.random.truncated_normal([windows_size, word_latent_factor, 1, num_filters], stddev=0.3),
                    name="user_aux_W")
    ...
    W2 = tf.Variable(tf.random.truncated_normal([windows_size, num_filters, 1, num_filters], stddev=0.3),
                     name="user_aux_W")
    ...
    o = tf.nn.max_pool(
        ...
        padding='VALID',
        name="user_pool")
    o = tf.compat.v1.squeeze(o)

    W1 = tf.Variable(tf.random.truncated_normal([num_filters, latent_dim], stddev=0.3), name="user_aux_W1")

    b1 = tf.Variable(tf.constant(0.1, shape=[latent_dim]), name="user_aux_b")

    user_aux_vector = tf.compat.v1.nn.relu_layer(o, W1, b1, name="user_layer1")
...
def highway_network(W_trans, b_trans, W, b, embed, drop_out):
    high_embed = tf.compat.v1.nn.relu_layer(embed, W_trans, b_trans)
...
def train_model():
    ...
    users_inputs = tf.compat.v1.placeholder(tf.int32, shape=[None, len(user_reviews.get(0))])
    items_inputs = tf.compat.v1.placeholder(tf.int32, shape=[None, len(item_reviews.get(0))])
    users_masks_inputs = tf.compat.v1.placeholder(tf.float32, shape=[None, len(user_masks.get(0))])
    items_masks_inputs = tf.compat.v1.placeholder(tf.float32, shape=[None, len(item_masks.get(0))])
    users_aux_inputs = tf.compat.v1.placeholder(tf.int32, shape=[None, len(user_aux_reviews.get(0))])
    users_aux_masks_inputs = tf.compat.v1.placeholder(tf.float32, shape=[None, len(user_aux_masks.get(0))])
    rates_inputs = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
    drop_out_rate = tf.compat.v1.placeholder(tf.float32)
    ...
    user_bias = tf.Variable(tf.random.normal([num_user, 1], mean=0, stddev=0.02), name="user_bias")
    item_bias = tf.Variable(tf.random.normal([num_item, 1], mean=0, stddev=0.02), name="item_bias")
    ...
    #highway network
    W_aux_trans = tf.Variable(tf.random.truncated_normal([num_factor, num_factor],stddev=0.3), name="user_aux_W_trans")
    b_aux_trans = tf.Variable(tf.constant(0.1, shape=[num_factor]), name="user_aux_b_trans")
    W_aux_high = tf.Variable(tf.random.truncated_normal([num_factor, num_factor], stddev=0.3), name="user_aux_W_high")
    ...
    #gated mechanism
    W_u_gated = tf.Variable(tf.random.truncated_normal([num_factor, num_factor], stddev=0.3), name="user_W_gated")
    W_aux_gated = tf.Variable(tf.random.truncated_normal([num_factor, num_factor], stddev=0.3), name="user_aux_W_gated")
    W_i_gated = tf.Variable(tf.random.truncated_normal([num_factor, num_factor], stddev=0.3), name="item_W_gated")
    ...
    W_u = tf.Variable(tf.random.truncated_normal([2 * num_factor, num_factor], stddev=0.3), name="user_W_gated")
    ...
    user_embeds_con = tf.compat.v1.nn.relu_layer(user_embeds_con, W_u, b_u)
    ...
    w_1 = tf.Variable(tf.random.truncated_normal([1, num_factor * 2],stddev=0.3), name="w_1")
    ...
    v = tf.Variable(tf.random.truncated_normal([num_factor * 2, 100],stddev=0.3), name="v")
    J_2 = tf.math.reduce_sum(tf.reduce_sum(tf.multiply(tf.matmul(embeds_sum_1, embeds_sum_2),tf.matmul(v, v, transpose_b=True)), 2), 1, keepdims=True)
    J_3 = tf.linalg.trace(tf.multiply(tf.matmul(embeds_sum_1, embeds_sum_2),
                                 tf.matmul(v, v, transpose_b=True)))
    predict_rating = (J_1 + 0.5 * (J_2 - J_3)) + user_bs + item_bs
    loss = tf.reduce_mean(tf.math.squared_difference(predict_rating, rates_inputs)) + gama * tf.nn.l2_loss(tf.subtract(user_embeds, user_aux_embeds))
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for e in range(epochs):
            ...
            loss_total = 0.0
            count = 0.0
            for i in range(len(user_input) // batch_size + 1):
...
def eval_model(users, items, users_inputs, items_inputs, users_aux_inputs, users_masks_inputs, items_masks_inputs, users_aux_masks_inputs, drop_out_rate, predict_rating, sess, user_test, item_test, user_input_test, item_input_test, user_aux_input_test, user_mask_input_test, item_mask_input_test, user_aux_mask_input_test, rating_input_test):
    ...
    for i in range(len(user_input_test) // 100 + 1):
        ...
        predict = sess.run(predict_rating, feed_dict={users: user_batch_test, items: item_batch_test, users_inputs: user_input_batch_test, items_inputs: item_input_batch_test, users_aux_inputs: user_aux_input_batch_test,
                                                      users_masks_inputs: user_mask_batch_test, items_masks_inputs: item_mask_batch_test, users_aux_masks_inputs: user_aux_mask_batch_test, drop_out_rate:0})
...
if __name__ == "__main__":
    ...
    # loading data
    firTime = time()
    dataset = Dataset(max_len, "/home/august/Desktop/FinalProject/PARL/data_digital_music_similar/")
